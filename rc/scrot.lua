...
-- obviously `scrot` must be installed
-- `xclip` must be installed to write to the clipboard
...
-- @param opts.filename The output file, or `false` to save to the clipboard.
...
function scrot.screenshot(opts)
   opts = opts or {}
   local filename = opts.filename == nil and filename_pattern or opts.filename
   local notify = opts.notify == nil and var.screenshot_notify or opts.notify

   local command = scrot_cmd
   if opts.select then
   ...
   if filename then
      command = command .. " --exec 'echo $f' --file '" .. filename .. "'"
   else
      command = command .. " --file - | xclip -i -selection clipboard -t image/png"
   end
   ...
   local callback = function(out, err, _, code)
      ...
      elseif notify then
         local dest = filename and out or "clipboard"
         naughty.notify{
            title="screenshot saved",
            text="written to " .. dest
         }
      end
   end
