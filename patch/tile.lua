...
local pad = beautiful.border_padding or 0
...
local function tile_group(cls, wa, orientation, fact, group)
   ...
   for c = group.first,group.last do
      local i = c - group.first +1
      geom[width] = size - (cls[c].border_width + pad) * 2
      geom[height] = math.floor(unused * fact[i] / total_fact) - (cls[c].border_width + pad) * 2
      geom[x] = group.coord + pad
      geom[y] = coord + pad
      geom = cls[c]:geometry(geom)
      coord = coord + geom[height] + cls[c].border_width * 2 + pad
      unused = unused - geom[height] - cls[c].border_width * 2 - pad
      total_fact = total_fact - fact[i]
      used_size = math.max(used_size, geom[width] + (cls[c].border_width + pad) * 2)
   end

   return used_size + pad
end
